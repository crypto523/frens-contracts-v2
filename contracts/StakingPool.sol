...
contract StakingPool is IStakingPool, Ownable {
  ...
  modifier maxTotDep(){
    require(msg.value + totalDeposits <= 32 ether, "total deposits cannot be more than 32 Eth");
    _;
  }
  ...
  address public artForPool;
  //address depositContractAddress = 0x00000000219ab540356cBB839Cbe05303d7705Fa; //mainnet
  address public depositContractAddress = 0xff50ed3d0ec03aC01D4C79aAd74928BFF48a7b2b; //goerli
  ...
  constructor(address owner_, bool validatorLocked_, IFrensPoolShare frensPoolShare_){
   ...
   function depositToPool() external payable noZeroValueTxn maxTotDep mustBeAccepting{
   ...
   function addToDeposit(uint _id) external payable maxTotDep mustBeAccepting{
     ...
     require(frensPoolShare.poolByIds(_id) == address(this), "wrong staking pool for id");
   ...
   function setPubKey(
     ...
     _setPubKey(_pubKey, _withdrawal_credentials, _signature, _deposit_data_root);
   }
   ...
   function _setPubKey(
     ...
     require(keccak256(_withdrawal_credentials) == keccak256(withdrawalCredFromAddr), "withdrawal credential mismatch");
     if(validatorLocked){
